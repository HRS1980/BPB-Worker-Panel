...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.hamid.get("proxySettings", {type: 'json'});
        warpConfigs = await env.hamid.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.hamid.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
    ...
    try {    
        await env.hamid.put("proxySettings", JSON.stringify(proxySettings));
